...
.NET 11 crypto additions are part of the evidence model. The `Aes` key-wrap methods (`EncryptKeyWrap`, `DecryptKeyWrap`, `TryDecryptKeyWrap`, `GetKeyWrapLength`) classify as an `AES Key Wrap` asset with `key-wrap`/`key-unwrap` operations under standard RFC 3394. The post-quantum algorithms classify as strong assets: `MLKem` is ML-KEM (family `key-agreement`, operation `key-agreement/encapsulate`, standard FIPS 203), `MLDsa` is ML-DSA (family `signature`, operation `sign`, standard FIPS 204), and `SlhDsa` is SLH-DSA (family `signature`, operation `sign`, standard FIPS 205). The experimental caller-driven TLS session types in `System.Net.Security` (`TlsContext`, `TlsSession`, `TlsBufferSession`, `TlsSocketSession`, `TlsOperationStatus`) classify as TLS protocol assets, and their use raises a low-severity `DOSAI-CRYPTO-EXPERIMENTAL-TLS-API` finding because the API surface is still marked experimental by diagnostic `SYSLIB5007`.
...
The document is schema-valid CycloneDX 1.6 JSON: it declares `$schema`, components and vulnerabilities reference themselves with the hyphenated `bom-ref` key, and every `cryptographic-asset` component carries `cryptoProperties.assetType` from the CycloneDX enum (`algorithm`, `certificate`, `protocol`, `related-crypto-material`). Where dosai evidence maps onto the spec, `algorithmProperties.primitive` is set from the asset family (for example `hash`, `mac`, `signature`, `kdf`, `key-agree`) and protocol assets get `protocolProperties.type` and `protocolProperties.version`; values with no clean CycloneDX equivalent stay in the `dosai:crypto:*` properties. Every reference emitted in `affects` and `dependencies` resolves to a component `bom-ref`, so strict validators accept the file.
