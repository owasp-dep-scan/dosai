...
`--debug` (or `DOSAI_DEBUG=1` / `DOSAI_DEBUG=true` in the environment, for callers such as cdxgen that cannot change arguments) turns on phase-granularity progress logging on stderr: resolved input path and runtime facts at startup, file-discovery counts per extension taken from the analyzers' own tree walk (with how many directories and files `--exclude` pruned), assembly scoping (kept/dropped and why), slow (> 1 s) assembly loads and load failures with the reason, Roslyn tree and reference counts per language, call graph node/edge counts after each build stage, the reachability bucketing path taken (condensed bitsets vs the budgeted walk fallback) with elapsed time, framework analyzer result counts, and output bytes written. While any phase runs longer than 30 seconds, a heartbeat names the innermost running phase with heap and working-set size, so a long run says where it is instead of appearing hung.

The log is designed to be pasted into an issue: it contains paths, counts, assembly and phase names, and timings only - never file contents, source text, string literals, secret or environment variable values, or child-process command lines (`--restore`/`--build` report the dotnet verb and exit code only). All output goes to stderr, so JSON artifacts and the `mcp` JSON-RPC stream on stdout are unaffected, and results are identical with and without the flag. When `--debug` is off (the default) each debug call is a single flag check and no messages are constructed.
...
```shell
dosai methods --path <repo> --exclude 'BuildOutput/**' '**/*.Designer.cs' --o methods.json
```
...
Call graph capture prefers Roslyn `IOperation` rather than syntax-only invocation matching. The operation walker records method calls, constructor calls, property gets and sets, delegate-style calls where represented by operations, and source spans. It also adds inferred evidence for virtual/interface dispatch, delegate and event callbacks, selected DI/framework registrations, service resolution, and simple reflection patterns. Every call graph edge is normalized so both endpoints exist as nodes. External calls become external nodes when Dosai cannot map them to a source declaration.

Directory source enumeration excludes `bin` and `obj` source files relative to the inspected root. Assembly enumeration remains valid for build and publish output directories, so binary-only use cases can still pass a `bin/Debug/...` path directly.
