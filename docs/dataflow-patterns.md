...
Supported pattern packs are `all`, `aspnet`, `data`, `filesystem`, `serialization`, `cloud`, `rpc`, `auth`, `crypto`, `grpc`, `messaging`, `ai`, `mcp`, `xss`, `xxe`, `injection`, `log`, `redos`, and `template`. See [Built-in data-flow pattern pack catalog](./pattern-packs.md) for the always-on defaults and the exact patterns added by each pack.
...
| `severity`          | no       | string                                                                            | Severity override for the slices this pattern produces: `info`, `low`, `medium`, or `high` (schema 4.1.0). Without it, severity defaults by sink category, and a match from a Low-confidence pattern is demoted one rank. |
